...
import { deleteWantedAction } from './actions';

export function DeleteButton({ id }: { id: string }) {
  const [pending, startTransition] = useTransition();

  function onClick() {
    if (!confirm(lt.wanted.form.deleteConfirm)) return;
    startTransition(async () => {
      const result = await deleteWantedAction(id);
      // Server Action redirects on success; we only land here on error.
      if (result?.error) {
        const errs = lt.wanted.errors as Record<string, string>;
        alert(errs[result.error] ?? lt.wanted.errors.server_error);
      }
    });
  }
  ...
  return (
    ...
    >
      {pending ? lt.common.loading : lt.wanted.delete}
    </button>
  );
}
