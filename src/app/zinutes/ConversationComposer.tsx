...
export function ConversationComposer({ threadKey }: { threadKey: string }) {
  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-2">
      <div className="flex items-end gap-2 rounded-xl border border-[var(--border)] bg-[var(--input)] p-2">
        <textarea
          name="body"
          maxLength={2000}
          rows={2}
          ...
          className="min-h-12 flex-1 resize-none rounded-lg border-0 bg-transparent px-3 py-2 text-base text-[var(--foreground)] placeholder:text-[var(--muted-soft)] focus:outline-none focus:ring-0 disabled:text-[var(--muted-foreground)]"
          disabled={pending}
        />
        <button
          type="submit"
