-- Every accepted auction bid must be at least EUR 50 above the displayed
-- price, and proxy bidding advances the visible price in EUR 50 steps.
...
begin
  ...
  if p_max_amount_eur < a.current_price_eur + 50 then
  ...
  next_price := case
    when second_max is null and a.bid_count = 0
      then least(top_bid.max_amount_eur, previous_price + 50)
    when second_max is null then previous_price
    else least(top_bid.max_amount_eur, second_max + 50)
  end;
